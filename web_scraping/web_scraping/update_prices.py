# for updating the prices collection, if anything changed (prices changed or new item appeared)
...
# filtering only to the last date 
df_old=df_old.sort_values('date').drop_duplicates('id',keep='last')
# yesterday = (date.today() - timedelta(days=1)).strftime('%m/%d/%Y')
# df_old=df_old.query('date == @yesterday')
...
# if the prices changed, and if the new price is not Nan 
df_filtered=df_updated[((df_updated.emag_price != df_updated.emag_new_price) & df_updated.emag_new_price.notnull() )
| ((df_updated.pc_garage_price != df_updated.pc_garage_price) & df_updated.pc_garage_new_price.notnull() )
| ((df_updated.cel_price != df_updated.cel_new_price) & df_updated.cel_new_price.notnull() )
| ((df_updated.flanco_price != df_updated.flanco_new_price) & df_updated.flanco_new_price.notnull() )]
